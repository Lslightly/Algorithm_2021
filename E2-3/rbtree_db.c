...
//	否则返回T.nil
Tree rbtreeFindInsertPlace(RBTree T, Key key, Tree *p)
{
	Tree x = T.root;
	while (x != T.nil)
	{
...
//	Insert Tree node z under its parent y
//	z's key must be initialized and z is malloced in heap
Tree rbTreeInsertUnderParent(RBTree *T, Tree z, Tree y) {
	if (T->size == 0) {
		z->color = BLACK;
		z->left = T->nil;
		z->right = T->nil;
...
Tree rbtreeInsertKey(RBTree *T, Key key)
{
	Tree y = T->nil;
	Tree x = T->root;
	if (T->size == 0)
	...
	{ //	没找到，找到了插入位置
		Tree z = (Tree)malloc(sizeof(Node));
		z->key = key;
		return rbTreeInsertUnderParent(T, z, y);
	}
	else
	{ //	在树上已经存在，只需count++
		return temp;
...
int main(void) {
	...
	for (int i = 0; i < n; i++) {
		...
		Tree p;
		Tree x = rbtreeFindInsertPlace(T, k, &p);
		if (x == T.nil) {
			Tree z = (Tree)malloc(sizeof(Node));
			z->key = k;
			rbTreeInsertUnderParent(&T, z, p);
			insert_result = 0;
		} else {
			insert_result = -1;
		}
